...
#define productUID "com.gmail.andreou.iotlabs:group_h"

Notecard notecard;

/*** Improved analog mic sampling for 8kHz ***/
#define INPUT_PIN       A0
#define SAMPLE_RATE     16000      // 8kHz sample rate
#define SAMPLE_INTERVAL 64         // Microseconds between samples (1000000/16000)

/** Fusion settings **/
#define FUSION_WINDOW 1

// circular buffer to hold last inference probability vector(s)
...
const float fusion_weights[FUSION_WINDOW] = { 1.0f };
// boost factors for each class
const float class_boost[EI_CLASSIFIER_LABEL_COUNT] = { 1.0f, 1.0f };
...
static bool microphone_inference_record(void) {
    ...
    const unsigned long timeout_ms = 10000;  // 10s timeout
...
void setup() {
    ...
    // Configure Notehub for continuous mode
...
void loop() {
    ...
    if (fusion_buffer_full) {
        ...
        // check knock threshold and send cloud note
        J *cloudReq = notecard.newRequest("note.add");
        JAddStringToObject(cloudReq, "file", "knock.qo");
        JAddBoolToObject(cloudReq, "sync", true);
        J *body = JAddObjectToObject(cloudReq, "body");

        if (S[0] > KNOCK_THRESHOLD) {
            Serial.println("KNOCK DETECTED!");
            if (body) {
                JAddStringToObject(body, "event", "KNOCK DETECTED");
            }
        }
        else {
            Serial.println("NO KNOCK DETECTED");
            if (body) {
                JAddStringToObject(body, "event", "NO KNOCK DETECTED");
            }
        }
        notecard.sendRequest(cloudReq);
